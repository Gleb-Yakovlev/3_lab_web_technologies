...
<table>
    <form action='' method='GET'>
        <tr>
            <th width=20%><button type='submit' name='POKAZAT'>Вывести таблицу экспонаты</button></th>
            <th width=20%><button type='submit' name='NEWDATA'>Создать новую запись</button></th>
        </tr>
    </form>
</table>
...
function seeTable()
{
    ...
    FROM exhibit ORDER BY Year_create DESC");
        ...
        <form action='' method='post'>
        <button type='submit' name='DELETE' value=$result[Code_exhibit]>Удалить</button>
        </form>
        

        <form action='' method='post'>
        <button type='submit' name='UPDATEDATA' value=$result[Code_exhibit]>Подтвердить изменения</button>
        </td>
